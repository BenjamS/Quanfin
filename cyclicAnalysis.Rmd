...
symb_currency_to <- "chf"
# OR stocks
stock_symbol <- NULL#"DBO RTY=F"#"NK=F"#"NG=F"#"FXI"#"SI=F"#"GC=F"#"NG=F"#"CL=F"#"EXS1.DE"#ZC=F#"HG=F" BNO CPER XLF "XLI", "XLK", "IYR", "XLV", "XLY", "XLP"
...
if(symbol_domain == "stocks"){
  ...
  if(time_step_unit == "weekly"){
    ...
    df_ohlcv <- stock_symbol %>% tq_get(get = "stock.prices",
                                        from = fromdate,
                                        periodicity = time_step) %>% as.data.frame()
    df_ohlcv$p <- df_ohlcv$adjusted
